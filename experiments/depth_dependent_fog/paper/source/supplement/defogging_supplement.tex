...
\input{table_rgb_benchmark_filtered.tex}
\FloatBarrier
...
The evaluated depth-informed extension retained that normalized base map
but used raw ZoeDepth output $z$ from the Intel ZoeDepth NYU--KITTI
model to modulate attenuation:
\[
\beta(\mathbf{x})=\mathrm{clip}\{\beta_0[1+0.70z(\mathbf{x})],0.03,8\}.
\]
The resulting transmission was
$T(\mathbf{x})=\exp[-\beta(\mathbf{x})d(\mathbf{x})]$. ZoeDepth maps
were precomputed as float16 arrays and were not normalized per image.
This branch is an evaluated generator extension rather than a new depth
estimator or atmospheric model.
...
\centering
\small
\caption{Depth-informed generator settings. All training-budget and
sampling settings matched the randomized branch.}
\label{tabS:depth_synth_params}
\begin{tabular}{lc}
\toprule
\textbf{Parameter} & \textbf{Value} \\
\midrule
depth source & Intel/zoedepth-nyu-kitti \\
base $\beta$ mean / variation & 0.59 / 0.55 \\
fog-field scale / octaves & 420 px / 3 \\
airlight RGB / variation & 0.72, 0.73, 0.76 / 0.05 \\
bloom strength / radius & 0.10 / 9 px \\
blur radius / fog coupling & 0.9 px / 0.22 \\
saturation mix / gamma & 0.24 / 0.96 \\
edge-veil / noise & 0.245 / 0.008 \\
raw-depth modulation weight & 0.70 \\
\bottomrule
\end{tabular}
\end{table}
...
light-fog probability & 0.30 \\
identity probability & 0.10 \\
airlight jitter & $\pm 0.03$ per channel \\
seed policy & stable path hash; random train offset \\
randomized validation / test PSNR & 21.56 / 21.61 dB \\
depth validation / test PSNR & 19.65 / 19.86 dB \\
\bottomrule
\end{tabular}
\end{table}

Matched evaluation used the same image order, crop policy, seed 734,
effective batch size 2, and 2{,}600-update budget for both generators
and all 30 architectures. Every randomized checkpoint improved on the
fixed randomized-fog test (median $+6.27$~dB) and every depth-informed
checkpoint improved on the fixed depth-fog test (median $+5.13$~dB).
Depth-informed checkpoints were lower than randomized checkpoints on the
randomized-fog test for all 30 models (median $-2.08$~dB), whereas they
exceeded randomized checkpoints on the depth-fog test for all 30
models (median $+3.41$~dB). Both branches reduced chamber performance.

\input{table_synthetic_strategy_30models.tex}

Five historical chamber checkpoints (DehazeFormer, DCPDN, GCANet,
Ancuti fusion, and GridDehazeNet) were retrained because their files were
no longer available. Table~\ref{tabS:rgb_benchmark} remains the
authoritative historical ranking; this new table supports matched
within-architecture strategy comparisons.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/supplement/figS_transfer_workflow_variant.pdf}
...
We evaluated chamber, randomized-synthetic, and depth-informed NAFNet
checkpoints on all available paired O-HAZE ($n=45$), NH-HAZE ($n=55$),
and released NTIRE nighttime images ($n=25$), without public-data
training or model selection. Native-resolution inference used
1024-pixel tiles with 96-pixel overlap. The depth-informed checkpoint
exceeded the randomized checkpoint by $+1.49$, $+0.63$, and
$+0.54$~dB, respectively, with higher SSIM on all three datasets.
Relative to the chamber checkpoint, depth-informed changes were
$+0.56$, $+0.67$, and $+1.78$~dB.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/supplement/figS_public_direct_transfer.pdf}
\caption{Complete-dataset direct-transfer comparison. Columns show the
hazy input, chamber checkpoint, randomized checkpoint, depth-informed
checkpoint, and paired target. Summary metrics use all available paired
images. The depth-informed extension exceeds the randomized branch on
mean PSNR and SSIM for all three datasets.}
...
The correctly loaded chamber-trained NAFNet reached 24.000~dB PSNR and
...
combines the requested tracking analysis with the corresponding
...
\caption{Controlled synthetic-camera sensitivity of fixed NAFNet
checkpoints. (a)~Left: condition-mean input-shift versus output-shift PSNR
...
95\% image-bootstrap confidence intervals. Blue denotes the correctly
loaded fog-chamber checkpoint. Orange denotes the archived
randomized-synthetic checkpoint as a descriptive sensitivity probe only:
its historical training path changed the output wrapper, so the two curves
do not constitute a controlled before/after fine-tuning comparison. Both
panels are vector graphics with embedded fonts.}
